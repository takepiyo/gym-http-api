...
public:
  ...
  boost::shared_ptr<Environment> make(const std::string &env_id,
                                      const std::string &render_mode) override;
};
  ...
  void step(const std::vector<float> &action, bool render,
    ...
    if (aspace->type == Space::DISCRETE) {
      ...
      assert(
          action.size() ==
          aspace->box_low
              .size()); // really assert, it's a programming error on C++ part
    ...
    save_state_here->terminated = ans["terminated"].asBool();
    save_state_here->truncated = ans["truncated"].asBool();
...
ClientReal::make(const std::string &env_id, const std::string &render_mode) {
  Json::Value req;
  req["env_id"] = env_id;
  req["render_mode"] = render_mode;
